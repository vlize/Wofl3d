...
void	ft_cast_xp(int *i, double *p1, double *k, t_env *env)
{
	...
	while (ft_limit_xy(i, env))
	{
		...
		p1[3] = p1[2] * k[0] + k[1];
		i[4] = p1[3] / BLOCK_SIZE;
		if ((i[2] != i[4]) && (ret = ft_cast0_x(i, p1, k, env)))
			return (ft_wall(i, &p1[4], ret, env));
		i[2] = i[4];
		if ((i[2] < env->map->yblock) && (i[2] >= 0))
		{
			if ((ret = ft_ray_cast(i, &p1[2], env)))
				return (ft_wall(i, &p1[2], ret, env));
			else
...
void	ft_cast_yp(int *i, double *p1, double *k, t_env *env)
{
	...
	while (ft_limit_xy(i, env))
	{
		...
		p1[2] = p1[3] * k[0] + k[1];
		i[3] = p1[2] / BLOCK_SIZE;
		if ((i[1] != i[3]) && (ret = ft_cast0_y(i, p1, k, env)))
			return (ft_wall(i, &p1[4], ret, env));
		i[1] = i[3];
